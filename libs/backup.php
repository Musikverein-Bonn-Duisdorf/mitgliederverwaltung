<?php
/**
 * Database backup / restore helpers (Melde-Parität MELD-90/131/132).
 *
 * Dumps only tables matching $dbprefix (mit_*). Never touches identity
...
function exportDatabaseSql() {
    ...
    $out = array();
    $out[] = '-- Mitgliederverwaltung database backup';
...
function createBackupZipFile() {
    ...
    $filename = 'mitgliederverwaltung-backup-'.$stamp.'.zip';
    $path = tempnam(sys_get_temp_dir(), 'mitbackup_');
    if($path === false) {
...
function backupAssertRestoreSafe($manifest, $sql) {
    ...
    if(is_array($manifest)) {
        if(isset($manifest['app']) && (string)$manifest['app'] !== '' && (string)$manifest['app'] !== 'mitgliederverwaltung') {
            throw new RuntimeException(
                'Restore refused: backup app is "'.$manifest['app'].'", expected mitgliederverwaltung.'
            );
        }
